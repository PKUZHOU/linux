			   ...
			   unsigned long *nr)
{
	...
	if (!(sysctl_numa_balancing_mode & NUMA_BALANCING_MEMORY_TIERING) && (!sc->may_swap || !can_reclaim_anon_pages(memcg, pgdat->node_id, sc))) {
...
static bool pgdat_balanced(pg_data_t *pgdat, int order, int highest_zoneidx)
{
	...
	if ((sysctl_numa_balancing_mode & NUMA_BALANCING_MEMORY_TIERING) && node_is_toptier(pgdat->node_id) && highest_zoneidx >= ZONE_NORMAL)
	...
	for (i = 0; i <= highest_zoneidx; i++) {
		...
		if (sysctl_numa_balancing_mode & NUMA_BALANCING_MEMORY_TIERING)
			...
			mark = wmark_pages(zone, WMARK_DEMOTE);
		else
...
bool pgdat_toptier_balanced(pg_data_t *pgdat, int order, int zone_idx)
{
	...
	count_vm_event(NR_PGDAT_TOPTIER_BALANCED);
				...
				unsigned int highest_zoneidx)
{
	...
	if (!remaining &&
		...
		if (!kthread_should_stop()){
			 ...
			 * not triggered.
			 */
			if ((sysctl_numa_balancing_mode & NUMA_BALANCING_MEMORY_TIERING) && node_is_toptier(pgdat->node_id) && pgdat->kswapd_failures >= MAX_RECLAIM_RETRIES) {
